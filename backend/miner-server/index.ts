...
function calculateHash(index: number, prevHash: string, timestamp: number, data: string): string {
    return crypto.createHash('sha256').update(index + prevHash + timestamp + data).digest('hex');
}

// Proof of Work
function proofOfWork(index: number, prevHash: string, timestamp: number, data: string, difficulty: number): { hash: string, nonce: number } {
    let nonce = 0;
    let hash: string;
...
function isValidNewBlock(newBlock: any, previousBlock: any): boolean {
    ...
    if (calculateHash(newBlock.index, newBlock.prevHash, newBlock.timestamp, newBlock.data + newBlock.nonce) !== newBlock.hash) return false;
...
io.on('connection', (socket) => {
    ...
    socket.on('mineBlock', (data) => {
        const lastBlock = localBlockchain.length > 0 ? localBlockchain[localBlockchain.length - 1] : null;
        const index = localBlockchain.length;
        const timestamp = Date.now();
        const difficulty = 4; // Define the difficulty level
    
        const { hash, nonce } = proofOfWork(index, lastBlock?.hash || '0', timestamp, data, difficulty);
